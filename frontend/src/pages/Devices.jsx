    ...
    <Layout>
      ...
      <div style={{ background:"#fff", borderRadius:12, boxShadow:"0 1px 4px rgba(0,0,0,0.06)", marginBottom:24, overflow:"hidden" }}>
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
          ...
          <tbody>
            {devices.length===0 && <tr><td colSpan={5} style={{ padding:24, color:"#94a3b8", textAlign:"center" }}>No devices yet.</td></tr>}
            {devices.map(d => (
              <tr key={d.id} style={{ borderBottom:"1px solid #f1f5f9" }}>
                <td style={{ padding:"12px 16px", fontSize:14, fontWeight:500 }}>{d.name}</td>
                ...
                {isAdmin && <td style={{ padding:"12px 16px" }}>
                  <button onClick={()=>handleDelete(d.id,d.name)} style={{ fontSize:13, padding:"5px 12px", background:"#fee2e2", color:"#dc2626", border:"none", borderRadius:6, cursor:"pointer" }}>Delete</button>
                </td>}
              </tr>
            ))}
          </tbody>
        ...
        <div style={{ background:"#fff", borderRadius:12, padding:24, boxShadow:"0 1px 4px rgba(0,0,0,0.06)" }}>
          ...
          <form onSubmit={handleAdd}>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr auto", gap:12, alignItems:"end" }}>
              ...
              <button type="submit" style={{ padding:"9px 20px", background:"#2563eb", color:"#fff", border:"none", borderRadius:8, fontWeight:600, cursor:"pointer" }}>Add</button>
            </div>
          </form>
        </div>
      )}
